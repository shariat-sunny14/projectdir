            ...
            with transaction.atomic():
                if (data['feature_id']).isnumeric() and int(data['feature_id']) > 0:
                    save_feature = feature_list.objects.filter(feature_id=data['feature_id']).update(feature_no=data['feature_no'], feature_name=data['feature_name'], feature_type=data['feature_type'], serial_no=data['serial_no'],
                                                                                                     feature_code=data['feature_code'], feature_page_link=data['feature_page_link'], command_name=data['feature_name'], module_id=moduleData, type_id=typeData, feature_icon=data['feature_icon'], is_active=data['feature_status'], ss_modifier=request.user)
                else:
                    save_feature = feature_list(feature_no=data['feature_no'], feature_name=data['feature_name'], feature_type=data['feature_type'], serial_no=data['serial_no'],
                                                feature_code=data['feature_code'], feature_page_link=data['feature_page_link'], command_name=data['feature_name'], module_id=moduleData, type_id=typeData, feature_icon=data['feature_icon'], is_active=data['feature_status'], ss_creator=request.user)
                    save_feature.save()
                resp['status'] = 'success'
                return JsonResponse({'success': True, 'msg': 'Successful'})
        except:
