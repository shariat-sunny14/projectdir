...
def receiveLocalPurchaseStockAPI(request):
    ...
    is_credit = int(data.get('is_credit', 0))
    is_cash = int(data.get('is_cash', 0))
    ...
    is_approved = int(data.get('is_approved', 0))
    ...
    try:
        with transaction.atomic():
            ...
            for item_id, qty, item_price, dis_percentage, batch, exp_dates in zip(item_ids, item_qtys, item_prices, dis_percentages, item_batchs, item_exp_dates):
                ...
                if (is_cash == 1 and is_approved == 1):
...
def localPurchaseDetailsReportsAPI(request):
    try:
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            ...
            local_purchases = local_purchase.objects.filter(id_org=org_id, is_approved=True, transaction_date__range=(start_date, end_date))
            ...
            for lp_data in local_purchases:
                ...
                data.append({
                    ...
                    'clients_name': lp_data.reg_id.full_name if lp_data.reg_id else lp_data.cus_clients_name,
                    'clients_phone': lp_data.reg_id.mobile_number if lp_data.reg_id else lp_data.cus_mobile_number,
