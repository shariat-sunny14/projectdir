...
def getOrganizationListAPI(request, org_id):
    ...
    try:
        ...
        org_Dtls.append({
            ...
            'start_up': org_list.is_start_up,
            'service_charge': org_list.service_charge,
            'alert_start': org_list.is_alert_start,
            'alert_end': org_list.is_alert_end,
            'is_active': org_list.is_active,
            'ser_char_alert': org_list.ser_char_alert,
            'is_delivery_chalan': org_list.is_delivery_chalan,
            'is_grn': org_list.is_grn,
            'is_lp': org_list.is_lp,
            'is_mrr': org_list.is_mrr,
            'is_store_trans': org_list.is_store_trans,
            'is_stock_recon': org_list.is_stock_recon,
...
def organization_addupdateAPI(request):
    ...
    try:
        with transaction.atomic():
            ...
            org_data.is_start_up = data.get('start_up')
            org_data.service_charge = data.get('service_charge')
            org_data.is_alert_start = data.get('alert_start')
            org_data.is_alert_end = data.get('alert_end')
            org_data.is_active = data.get('is_active', 0)
            org_data.ser_char_alert = data.get('ser_char_alert', 0)
            org_data.is_delivery_chalan = data.get('is_delivery_chalan', 0)
            org_data.is_grn = data.get('is_grn', 0)
            org_data.is_lp = data.get('is_lp', 0)
            org_data.is_mrr = data.get('is_mrr', 0)
            org_data.is_store_trans = data.get('is_store_trans', 0)
            org_data.is_stock_recon = data.get('is_stock_recon', 0)
            ...
            try:
                all_users = User.objects.filter(org_id=org_data.org_id)
                for u in all_users:
                    save_user_context_to_json(u)
                    save_org_service_charge_to_json(u)
...
def branchAddUpdateAPI(request):
    ...
    try:
        with transaction.atomic():
            ...
            try:
                from user_auth.utils.save_user_context import save_user_context_to_json
                all_users = User.objects.filter(branch_id=branch_data.branch_id)
                for u in all_users:
                    save_user_context_to_json(u)
            except Exception as context_err:
                logger.warning(f"Bulk context update failed: {str(context_err)}")
