...
def paymentDetailsDeleteAPI(request, pay_id):
    if request.method == 'DELETE':
        try:
            # Get all the matching chalan details
            payment_DtlIDs = paymentsdtls.objects.filter(pay_id=pay_id)
            if payment_DtlIDs.exists():
                # Delete the current payment_DtlIDs entry
                payment_DtlIDs.delete()
                return JsonResponse({'success': True, 'msg': 'Successfully deleted'})
            else:
                return JsonResponse({'success': False, 'errmsg': f'Payments details with ID {pay_id} not found.'})
        except Exception as e:
            return JsonResponse({'success': False, 'errmsg': f'Error occurred: {str(e)}'})