...
@login_required()
def othersBankBkashEtcCollectionSummaryReportAPI(request):
...
@login_required()
def previousRemainingFromTotalNetCollectionAPI(request):
    ...
    if request.method == "POST":
        try:
            ...
            # আজকের তারিখ ও একদিন আগের তারিখ
            today = timezone.now().date()
            previous_day = today - timedelta(days=1)

            # Filter করার জন্য datetime range বানানো
            start_from = datetime.combine(previous_day, datetime.min.time())
            end_from = datetime.combine(previous_day, datetime.max.time())
            ...
            data = {
                'previous_total_remaining_collection': total_remaining_collection,
            }
...
@login_required()    
def presentToSevenDaysCollectionSummaryReportAPI(request):
    # Static date range: Today + Last 7 Days (8 days total)
    ...
    try:
        ...
        for current_date in date_range:
            start_from = datetime.combine(current_date, datetime.min.time())
            end_from = datetime.combine(current_date, datetime.max.time())

            # Initialize daily variables
            total_collection = total_due_collection = total_grand_collection = 0
            total_refund_collection = total_adjust_collection = 0
            grand_total_net_collection = 0
            grandtotal_sales = grand_total_discount = grand_total_vat_tax = 0
            grand_total_cancel_amt = grand_total_net_bill = 0
            total_other_exp_amt = grandtotal_cost = 0
            total_daily_deposit = deposit_main_branch = dep_rec_sub_branch = 0
            total_cash_on_hand = 0
            grand_mobile_bank_coll = grand_total_bank_coll = 0
            grand_woinvcoll_amt = grand_woinvcoll_mb_amt = grand_woinvcoll_b_amt = 0
            grand_regclipay_amt = grand_regclipay_mb_amt = grand_regclipay_b_amt = 0
            grand_suppclipay_amt = grand_suppclipay_mb_amt = grand_suppclipay_b_amt = 0
            net_sub_total_coll_cash = total_remaining_collection = 0
            grand_carrying_pay_amt = grand_total_amt = grand_ret_total_amt = grand_mrr_total_amt = 0
            grand_bkash_coll_amt = grand_nagad_coll_amt = grand_upay_coll_amt = grand_ucash_coll_amt = 0
            grand_bkash_ref_amt = grand_nagad_ref_amt = grand_upay_ref_amt = grand_ucash_ref_amt = 0
            grand_check_coll_amt = grand_debit_card_coll_amt = grand_credit_card_coll_amt = 0
            grand_bank_deposit_coll_amt = 0
            grand_check_ref_amt = grand_debit_card_ref_amt = grand_credit_card_ref_amt = 0
            grand_bank_deposit_ref_amt = 0

            # Fetch data for current day
            payments = payment_list.objects.filter(
                pay_date__range=(start_from, end_from),
                inv_id__org_id=org_id,
                inv_id__branch_id=branch_id
            ).all()

            other_rent_exp = rent_others_exps.objects.filter(
                other_exps_date__range=(start_from, end_from),
                org_id=org_id, branch_id=branch_id, is_carrying_cost=False
            ).all()

            carrying_cost_buyer = rent_others_exps.objects.filter(
                is_buyer=True, org_id=org_id, branch_id=branch_id, is_carrying_cost=True
            ).all()

            invoice_details = invoicedtl_list.objects.filter(
                inv_id__org_id=org_id, inv_id__branch_id=branch_id
            ).all()

            bank_deposits = daily_bank_statement.objects.filter(
                deposit_date__range=(start_from, end_from),
                is_bank_statement=True, org_id=org_id, branch_id=branch_id
            ).all()

            daily_cash_on_hand = cash_on_hands.objects.filter(org_id=org_id, branch_id=branch_id)

            local_purchases = local_purchase.objects.filter(
                transaction_date__range=(start_from, end_from),
                is_approved=True, is_cash=True, id_org=org_id, branch_id=branch_id
            )
            ...
            manual_return_rec = manual_return_receive.objects.filter(
                transaction_date__range=(start_from, end_from),
                is_approved=True, is_cash=True, id_org=org_id, branch_id=branch_id
            )
            ...
            submit_main_branch = daily_bank_statement.objects.filter(
                deposit_date__range=(start_from, end_from),
                is_branch_deposit=True, org_id=org_id, branch_id=branch_id
            ).all()

            deposit_rec_sub_branch = daily_bank_statement.objects.filter(
                deposit_date__range=(start_from, end_from),
                is_branch_deposit_receive=True, org_id=org_id, branch_id=branch_id
            ).all()

            carrying_cost_payment = carrying_cost_payments.objects.filter(
                c_cost_date__range=(start_from, end_from), org_id=org_id, branch_id=branch_id
            ).all()

            without_inv_coll = without_invoice_collection.objects.filter(
                coll_date__range=(start_from, end_from), org_id=org_id, branch_id=branch_id
            ).all()

            # registration client
            reg_client_payment = paymentsdtls.objects.filter(
                pay_date__range=(start_from, end_from), org_id=org_id, branch_id=branch_id, is_reg_client=True
            ).all()

            # supplier client
            supplier_client_payment = paymentsdtls.objects.filter(
                pay_date__range=(start_from, end_from), org_id=org_id, branch_id=branch_id, is_supplier_party=True
            ).all()

            # carrying cost payments
            for carrying_pay in carrying_cost_payment:
                carrying_pay_amt = carrying_pay.c_cost_amt or 0
                grand_carrying_pay_amt += carrying_pay_amt
                grand_carrying_pay_amt = round(grand_carrying_pay_amt, 2)
            ...
            # Sum the deposit rec from sub branch over the date range
            dep_rec_sub_branch = deposit_rec_sub_branch.aggregate(
                s=Sum('deposits_amt')
            )['s'] or 0  # Return 0 if no deposits are found

            # Sum the deposit of the main branch over the date range
            deposit_main_branch = submit_main_branch.aggregate(
                s=Sum('deposits_amt')
            )['s'] or 0  # Return 0 if no deposits are found
            ...
            # other rent or expence
            for otherExp in other_rent_exp:
                other_exp_amt = otherExp.other_exps_amt or 0
                total_other_exp_amt += other_exp_amt
                total_other_exp_amt = round(total_other_exp_amt, 2)

            # Calculate carrying cost from buyer per inv_id
            inv_ids = payments.values_list('inv_id', flat=True)
            buyer_total_cost = carrying_cost_buyer.filter(inv_id__in=inv_ids).values('inv_id').annotate(
                total_cost=Sum(F('other_exps_amt'), output_field=FloatField())
            )
            ...
            if invoice_details.exists():
                # Calculate total sales per inv_id
                sales_totals = invoice_details.filter(inv_id__in=inv_ids).values('inv_id').annotate(
                    total_sales=Sum(F('sales_rate') * F('qty'), output_field=FloatField())
                )

                item_w_discount = invoice_details.filter(inv_id__in=inv_ids).values('inv_id').annotate(
                    item_disc=Sum(((F('item_w_dis') / F('qty')) * ((F('qty') - F('is_cancel_qty')))), output_field=FloatField())
                )

                gross_discount = invoice_details.filter(inv_id__in=inv_ids).values('inv_id').annotate(
                    gross_disc=Sum(((F('gross_dis') / F('qty')) * ((F('qty') - F('is_cancel_qty')))), output_field=FloatField())
                )

                item_w_gross_vat_tax = invoice_details.filter(inv_id__in=inv_ids).values('inv_id').annotate(
                    total_vat_tax=Sum(((F('gross_vat_tax') / F('qty')) * ((F('qty') - F('is_cancel_qty')))), output_field=FloatField())
                )

                cancel_amt = invoice_details.filter(inv_id__in=inv_ids).values('inv_id').annotate(
                    total_cancel_amt=Sum((F('sales_rate') * F('is_cancel_qty')), output_field=FloatField())
                )
            else:
                # Set all totals to 0.0 if no invoice details are found
            ...
            collections_by_inv_id = defaultdict(lambda: {
                ...
                'total_debit_coll_amt': 0.0,
                'total_credit_coll_amt': 0.0,
                'total_bank_deposit_coll_amt': 0.0,
                'total_check_refund_amt': 0.0,
                'total_debit_refund_amt': 0.0,
                'total_credit_refund_amt': 0.0,
                'total_bank_deposit_refund_amt': 0.0,
                'total_collection_amt_cash': 0.0,
                'total_due_collection_amt_cash': 0.0,
                'total_refund_collection_amt_cash': 0.0,
            })
            ...
            pay_mode_bank_key_refund = {
                ...
                "3": 'total_debit_refund_amt',
                "4": 'total_credit_refund_amt',
                "9": 'total_bank_deposit_refund_amt',
            }
            ...
            for inv_id, collections in collections_by_inv_id.items():
                # Get total sales for this inv_id from the annotated values
                try:
                    sales_total_entry = next(s for s in sales_totals if s['inv_id'] == inv_id)
                    total_sales = sales_total_entry['total_sales'] if sales_total_entry else 0
                    collections['total_sales'] = total_sales
                    grandtotal_sales += total_sales
                ...
                try:
                    cost_entry = next(s for s in buyer_total_cost if s['inv_id'] == inv_id)
                    total_cost = cost_entry['total_cost'] if cost_entry else 0
                    collections['total_cost'] = total_cost
                    grandtotal_cost += total_cost
                except StopIteration:
                ...
                # item wise discount
                try:
                    item_disc = next(s for s in item_w_discount if s['inv_id'] == inv_id)['item_disc']
                    collections['item_disc'] = item_disc
                    item_disc = round(item_disc, 2)
                except StopIteration:
                    item_disc = 0
                ...
                # gross discount
                try:
                    gross_disc = next(s for s in gross_discount if s['inv_id'] == inv_id)['gross_disc']
                    collections['gross_disc'] = gross_disc
                    gross_disc = round(gross_disc, 2)
                except StopIteration:
                    gross_disc = 0
                ...
                # total vat tax
                try:
                    total_vat_tax = next(s for s in item_w_gross_vat_tax if s['inv_id'] == inv_id)['total_vat_tax']
                    collections['total_vat_tax'] = total_vat_tax
                    total_vat_tax = round(total_vat_tax, 2)
                    # grand total vat tax
                    grand_total_vat_tax += total_vat_tax
                ...
                # total cancel amount
                try:
                    total_cancel_amt = next(s for s in cancel_amt if s['inv_id'] == inv_id)['total_cancel_amt']
                    collections['total_cancel_amt'] = total_cancel_amt
                    total_cancel_amt = round(total_cancel_amt, 2)
                    # grand total cancel amount
                    grand_total_cancel_amt += total_cancel_amt
                ...
                ################# mobile bank collection #######################
                # bkash coll amt
                grand_bkash_coll_amt += collections['total_bkash_coll_amt']
                grand_bkash_coll_amt = round(grand_bkash_coll_amt, 2)
                # nagad coll amt
                grand_nagad_coll_amt += collections['total_nagad_coll_amt']
                grand_nagad_coll_amt = round(grand_nagad_coll_amt, 2)
                # upay coll amt
                grand_upay_coll_amt += collections['total_upay_coll_amt']
                grand_upay_coll_amt = round(grand_upay_coll_amt, 2)
                # ucash coll amt
                grand_ucash_coll_amt += collections['total_ucash_coll_amt']
                grand_ucash_coll_amt = round(grand_ucash_coll_amt, 2)

                ################# mobile bank Refund #######################
                # bkash Refund amt
                grand_bkash_ref_amt += collections['total_bkash_refund_amt']
                grand_bkash_ref_amt = round(grand_bkash_ref_amt, 2)
                # nagad Refund amt
                grand_nagad_ref_amt += collections['total_nagad_refund_amt']
                grand_nagad_ref_amt = round(grand_nagad_ref_amt, 2)
                # upay Refund amt
                grand_upay_ref_amt += collections['total_upay_refund_amt']
                grand_upay_ref_amt = round(grand_upay_ref_amt, 2)
                # ucash Refund amt
                grand_ucash_ref_amt += collections['total_ucash_refund_amt']
                grand_ucash_ref_amt = round(grand_ucash_ref_amt, 2)

                ################# bank collection #######################
                # Check coll amt
                grand_check_coll_amt += collections['total_check_coll_amt']
                grand_check_coll_amt = round(grand_check_coll_amt, 2)
                # Debit Card coll amt
                grand_debit_card_coll_amt += collections['total_debit_coll_amt']
                grand_debit_card_coll_amt = round(grand_debit_card_coll_amt, 2)
                # Credit Card coll amt
                grand_credit_card_coll_amt += collections['total_credit_coll_amt']
                grand_credit_card_coll_amt = round(grand_credit_card_coll_amt, 2)
                # Bank Deposit coll amt
                grand_bank_deposit_coll_amt += collections['total_bank_deposit_coll_amt']
                grand_bank_deposit_coll_amt = round(grand_bank_deposit_coll_amt, 2)

                ################# bank Refund #######################
                # Check Refund amt
                grand_check_ref_amt += collections['total_check_refund_amt']
                grand_check_ref_amt = round(grand_check_ref_amt, 2)
                # Debit Card Refund amt
                grand_debit_card_ref_amt += collections['total_debit_refund_amt']
                grand_debit_card_ref_amt = round(grand_debit_card_ref_amt, 2)
                # Credit Card Refund amt
                grand_credit_card_ref_amt += collections['total_credit_refund_amt']
                grand_credit_card_ref_amt = round(grand_credit_card_ref_amt, 2)
                # Bank Deposit Refund amt
                grand_bank_deposit_ref_amt += collections['total_bank_deposit_refund_amt']
                grand_bank_deposit_ref_amt = round(grand_bank_deposit_ref_amt, 2)
            ...
            # bank collection
            total_bank_coll_amt = grand_check_coll_amt + grand_debit_card_coll_amt + grand_credit_card_coll_amt + grand_bank_deposit_coll_amt
            net_bank_coll_amt = ((total_bank_coll_amt + grand_woinvcoll_b_amt) - (grand_regclipay_b_amt + grand_suppclipay_b_amt))

            # bank refund and grand collection 
            total_bank_refund_amt = grand_check_ref_amt + grand_debit_card_ref_amt + grand_credit_card_ref_amt + grand_bank_deposit_ref_amt
            grand_bank_coll = net_bank_coll_amt - total_bank_refund_amt
            grand_total_bank_coll = round(grand_bank_coll, 2)
            ...
            summary_by_date.append({
                ...
                'total_adjust_collection': round(total_adjust_collection, 2),
                'grand_total_net_collection': round(grand_total_net_collection, 2),
                'grandtotal_sales': round(grandtotal_sales, 2),
                'grand_total_discount': round(grand_total_discount, 2),
                'grand_total_vat_tax': round(grand_total_vat_tax, 2),
                'grand_total_cancel_amt': round(grand_total_cancel_amt, 2),
                'grand_total_net_bill': round(grand_total_net_bill, 2),
                ...
                'grand_total_collection': round(grand_total_collection, 2),
                'total_remaining_collection': round(total_remaining_collection, 2),
                'grandtotal_cost': round(grandtotal_cost, 2),
                'total_daily_deposit': round(total_daily_deposit, 2),
                'deposit_main_branch': round(deposit_main_branch, 2),
                'dep_rec_sub_branch': round(dep_rec_sub_branch, 2),
                ...
                'grand_upay_ref_amt': grand_upay_ref_amt,
                'grand_ucash_ref_amt': grand_ucash_ref_amt,
                'grand_check_coll_amt': grand_check_coll_amt,
                'grand_debit_card_coll_amt': grand_debit_card_coll_amt,
                'grand_credit_card_coll_amt': grand_credit_card_coll_amt,
                'grand_bank_deposit_coll_amt': grand_bank_deposit_coll_amt,
                'grand_check_ref_amt': grand_check_ref_amt,
                'grand_debit_card_ref_amt': grand_debit_card_ref_amt,
                'grand_credit_card_ref_amt': grand_credit_card_ref_amt,
                'grand_bank_deposit_ref_amt': grand_bank_deposit_ref_amt,
            })
        ...
        print("Error in presentToSevenDaysCollectionSummaryReportAPI:", e)
        return JsonResponse({'error': str(e)}, status=500)