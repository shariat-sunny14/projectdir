...
from photo_gallery.utils import compress_image
...
def saveAdvertItemAPI(request):
    if request.method == 'POST':
        try:
            ...
            # compress before saving — cuts a big phone/DSLR upload down to a
            # web-friendly size (see photo_gallery/utils.py for the same
            # treatment already used on gallery photo uploads)
            if banner_img:
                banner_img = compress_image(banner_img)
    ...
    if request.method == 'POST':
        try:
            ...
            if banner_img:
                ...
                # compress before saving — same treatment as the create path above
                banner_obj.banner_img = compress_image(banner_img)
