...
def getPostorderUpdateInvoiceDtlsAPI(request):
    ...
    try:
        ...
        invoice_data = {
            ...
            'side_office_factory': invoice.side_office_factory,
    ...
    try:
        with transaction.atomic():
            ...
            if inv_id:
                ...
                invoice.side_office_factory = data['side_off_factory']
                ...
                if notapp_carr_cost == "1":
                    ...
                    resp['msg'] = 'Carrying Cost record deleted as not applicable.'
                else:
                    # Check if both seller and buyer are False/0
                    if not seller and not buyer:
                        resp['errmsg'] = 'Carrying Cost Mode Buyer, Seller not Selected Found. Plzz.. Select Carrying Cost Mode'
                        return JsonResponse(resp)
        
                    # Delete rent_others_exps record if exists to avoid duplicates
                    rent_others_exps.objects.filter(inv_id=invoice).delete()
                    # Create or update rent_others_exps new record
                    rent_others_exps.objects.update_or_create(
                        inv_id=invoice,
                    ...
                    resp['msg'] = 'Carrying Cost Updated Successfully'
