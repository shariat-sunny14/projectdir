...
def getClientsTransactionsSummaryAPI(request):
    ...
    if request.method == "GET":
        ...
        payments_details = paymentsdtls.objects.filter(reg_id=int(reg_id), is_reg_client=True)
        if org_id and org_id.isdigit():
        ...
        woinvcoll_details = without_invoice_collection.objects.filter(reg_id=int(reg_id))
        if org_id and org_id.isdigit():
        ...
        for lp_data in local_purchases:
            ...
            lp_returndtls = lp_return_details.objects.filter(lp_id=lp_data, is_returned=True).all()
            ...
            lp_returndtls_cancel = lp_return_details.objects.filter(lp_id=lp_data, is_canceled=True).all()
            ...
            for lpreturnCancel in lp_returndtls_cancel:
                ...
                if lpreturnQtyCancel > 0:
                    # Get corresponding local_purchasedtl entry for unit price and discount
                    lp_dtlCancel = lpdata_details.filter(lp_id=lp_data, item_id=lpreturnCancel.item_id).first()
                    if not lp_dtlCancel:
                        continue  # Skip if no matching purchase detail found
                    ...
                    # Calculate the discount
                    total_return_disamt_cancel = total_returnamt_cancel * ((lp_dtlCancel.dis_percentage or 0) / 100)
...
def getClientsTransactionsDetailsAPI(request):
    try:
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            ...
            manu_return_receive = manual_return_receive.objects.filter(transaction_date__range=(data_from_date, data_to_date), id_org=org_id, reg_id=reg_id, is_approved=True).all()
            manu_return_receivedtls = manual_return_receivedtl.objects.all()
            local_purchases = local_purchase.objects.filter(transaction_date__range=(data_from_date, data_to_date), id_org=org_id, reg_id=reg_id, is_approved=True).all()
            lpdata_details = local_purchasedtl.objects.all()
            lp_return_datadtls = lp_return_details.objects.all()

            # Filter credited opening transactions
            opening_credited = opening_balance.objects.filter(opb_date__range=(data_from_date, data_to_date), reg_id=int(reg_id), is_credited=True)
            if org_id and org_id.isdigit():
                opening_credited = opening_credited.filter(org_id=int(org_id))
            ...
            # Filter debited opening transactions
            opening_debited = opening_balance.objects.filter(opb_date__range=(data_from_date, data_to_date), reg_id=int(reg_id), is_debited=True)
            if org_id and org_id.isdigit():
                opening_debited = opening_debited.filter(org_id=int(org_id))
            for open_deb in opening_debited:
                data.append({
            ...
            # payments transections
            payments_details = paymentsdtls.objects.filter(pay_date__range=(data_from_date, data_to_date), reg_id=int(reg_id), is_reg_client=True)
            if org_id and org_id.isdigit():
                payment_dtls = payments_details.filter(org_id=int(org_id))
            for paydtls in payment_dtls:
                data.append({
            ...
            woinvcoll_details = without_invoice_collection.objects.filter(coll_date__range=(data_from_date, data_to_date), reg_id=int(reg_id))
            if org_id and org_id.isdigit():
                woinvcoll_dtls = woinvcoll_details.filter(org_id=int(org_id))
            for woinvcoll in woinvcoll_dtls:
                data.append({
            ...
            invoice_datas = invoice_list.objects.filter(invoice_date__range=(data_from_date, data_to_date), reg_id=int(reg_id))
