...
class serviceChargePayment(models.Model):
    service_id = models.BigAutoField(primary_key=True, default=1955500008000, editable=False)
    creater_date = models.DateField(default=timezone.now, editable=False)
    service_month = models.CharField(max_length=20, null=True, blank=True)
    service_year = models.CharField(max_length=30, null=True, blank=True)
    service_amt = models.IntegerField(null=True, blank=True)
    service_payment = models.IntegerField(null=True, blank=True)
    payment_trans_id = models.CharField(max_length=100, null=True, blank=True)
    reference = models.CharField(max_length=30, null=True, blank=True)
    org_id = models.ForeignKey(organizationlst, null=True, blank=True, related_name='org_id2service', on_delete=models.DO_NOTHING)
    branch_id = models.ForeignKey(branchslist, null=True, blank=True, related_name='branch_id2service', on_delete=models.DO_NOTHING)
    is_passing = models.BooleanField(default=False)
    is_approved = models.BooleanField(default=False)
    ss_creator = models.ForeignKey(User, null=True, blank=True, related_name='ss_creator2service', on_delete=models.DO_NOTHING, editable=False)
    ss_created_on = models.DateTimeField(auto_now_add=True)
    ss_created_session = models.BigIntegerField(null=True, blank=True, default=1672000000800, editable=False)
    ss_modifier = models.ForeignKey(User, null=True, blank=True, related_name='ss_modifier2service', on_delete=models.DO_NOTHING)
    ss_modified_on = models.DateTimeField(auto_now=True)
    ss_modified_session = models.BigIntegerField(null=True, blank=True, default=1789000000900, editable=False)

    def save(self, *args, **kwargs):
        last_data = serviceChargePayment.objects.all()

        if last_data.exists() and self._state.adding:
            last_order = last_data.latest('service_id')
            user_session = last_data.latest('ss_created_session')
            modifier_session = last_data.latest('ss_modified_session')
            self.service_id = int(last_order.service_id) + 1
            self.ss_created_session = int(user_session.ss_created_session) + 1
            self.ss_modified_session = int(
                modifier_session.ss_modified_session) + 1
        super().save(*args, **kwargs)
