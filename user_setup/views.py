...
def getUserListsAPI(request):
    ...
    is_version = request.GET.get('is_version', None)

    # Initialize Q object for dynamic filtering
    filter_q = Q()

    if is_active is not None:
        # Assuming is_active is passed as string 'true' or 'false'
        if is_active.lower() == 'true':
            filter_q &= Q(is_active=True)
        elif is_active.lower() == 'false':
            filter_q &= Q(is_active=False)

    if org_id and org_id.lower() != 'none':
        filter_q &= Q(org_id=org_id)

    if branch_id and branch_id.lower() != 'none':
        filter_q &= Q(branch_id=branch_id)

    if is_version:
        if is_version == 'english':
            filter_q &= Q(is_english=True)
        elif is_version == 'bangla':
            filter_q &= Q(is_bangla=True)

    # Apply the filters
    user_data = user_list.filter(filter_q)
...
def saveUserAPI(request):
    ...
    if request.method == "POST":
        ...
        is_english = request.POST.get('is_english', 0)
        is_bangla = request.POST.get('is_bangla', 0)
...
def useraccessManageAPI(request):
    ...
    if user_id and user_id.isnumeric():
        ...
        if user_obj:
            ...
            sectionlist = in_section.objects.filter(is_active=True, is_english=user_obj.is_english, is_bangla=user_obj.is_bangla)
