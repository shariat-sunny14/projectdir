...
def load_service_charge_context_from_json(request):
    context = {}
    if request.user.is_authenticated:
        try:
            file_path = os.path.join(settings.BASE_DIR, 'service_charge_data', f'service_charge_{request.user.user_id}.json')
            if os.path.exists(file_path):
                with open(file_path, 'r', encoding='utf-8') as f:
                    context = json.load(f)
        except Exception as e:
            context['service_charge_error'] = str(e)

    return context



def load_notifications_item_count_from_json(request):
    context = {}
    if request.user.is_authenticated:
        try:
            file_path = os.path.join(settings.BASE_DIR, 'notification_data', 'notification_data.json')
            if os.path.exists(file_path):
                with open(file_path, 'r', encoding='utf-8') as f:
                    context = json.load(f)
        except Exception as e:
