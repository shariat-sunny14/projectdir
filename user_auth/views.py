        ...
        if user is not None and user.check_password(password):
            try:
                ...
                if user.org_id != organization.org_id:
                    resp = {'success': False, 'errmsg': 'Invalid organization.'}
                    return JsonResponse(resp)

                if user.expiry_date and user.expiry_date < present_date:
                    resp = {'success': False, 'errmsg': 'User account has expired. Please contact the administration.'}
                    return JsonResponse(resp)

                # Only for non-superuser: block if alert_end_date < today or months missing
                if not user.is_superuser:
                    ...
                    alert_end_date = date(
                        next_year,
                        next_month,
                        min(is_alert_end, calendar.monthrange(next_year, next_month)[1])
                    )
                    ...
                    if is_start_up:
                        ...
                        last_pointer = date(end_year, end_month, 1)

                        while current_pointer <= last_pointer:
                            y, m = current_pointer.year, current_pointer.month
                            all_required_months.append((y, MONTHS_LIST[m - 1]))
                            current_pointer = date(y + 1, 1, 1) if m == 12 else date(y, m + 1, 1)

                    approved_or_passed_payments = serviceChargePayment.objects.filter(
                        org_id=organization,
                        service_year__isnull=False,
                        service_month__isnull=False
                    ).filter(
                        Q(is_approved=True) | Q(is_passing=True)
                    )

                    paid_months = set(
                        (int(p.service_year), p.service_month.lower().strip())
                        for p in approved_or_passed_payments
                    )

                    missing_months = [m for m in all_required_months if m not in paid_months]

                    if alert_end_date < today or missing_months:
                        resp = {
                            'success': False,
                            'errmsg': 'Blocked due to missing service charge payment.',
                ...
                resp = {'success': False, 'errmsg': 'Organization not found.'}
        else:
            resp = {'success': False, 'errmsg': 'Invalid Username and Password. Please Try Again.'}
