...
from what_we_do.models import Service, ServiceSection
from who_we_are.models import WhoWeAre
import yagmail
...
def websitesManagerAPI(request):
    ...
    posts = FacebookPost.objects.exclude(message__isnull=True, image__isnull=True).order_by('-created_time')
    ...
    stats = who_we_are.stats.all()
    
    # ---- What We Do (Services) ----
    service_section = ServiceSection.load()
    services = Service.objects.filter(is_active=True).order_by('number')
    
    # ---- How It Works (Process) ----
    process_section = ProcessSection.load()
    ...
    # EnrollList queryset with prefetch — ordered by the admin-settable
    # order_by field (lower shows first on the website), enroll_id as a
    # tiebreaker for equal/default order_by values
    enroll_data = EnrollList.objects.prefetch_related(
        Prefetch('enroll2enrolldtls', queryset=details_qs, to_attr='published_details')
    ).order_by('order_by', 'enroll_id')
...
def organization_getAPI(request):
    try:
        ...
        data = {
            ...
            'youtube_link': org_data.youtube_link,
...
def organization_addupdateAPI(request):
    ...
    try:
        with transaction.atomic():
            ...
            org_data.youtube_link = data.get('youtube_link')
            ...
            if new_logo_uploaded:
                ...
                # ======================
                # GIF → SAVE ORIGINAL (no processing, animation-safe)
                # ======================
                if file_ext == '.gif':
                ...
                # ======================
                # IMAGE FILE → RESIZE (format preserved via extension)
                # ======================
                elif file_ext in ['.jpg', '.jpeg', '.png', '.webp']:

                    # Extension → PIL format mapping.
                    # NOTE: image.format becomes None after .convert(),
                    # so we NEVER rely on image.format here — extension decides format.
                    ext_to_format = {
                        '.jpg': 'JPEG',
                        '.jpeg': 'JPEG',
                        '.png': 'PNG',
                    ...
                    image_format = ext_to_format[file_ext]

                    image = Image.open(logo_file)

                    # JPEG doesn't support alpha/transparency, so only convert
                    # to RGB when target format is JPEG. PNG/WEBP keep their
                    # original mode (transparency preserved).
                    ...
                    # quality only applies to JPEG/WEBP, not PNG
                ...
                # ======================
                # OTHER FILE TYPES → SAVE ORIGINAL
                # ======================
                else:
                    filename = default_storage.save(
                        f'org_logos/{logo_file.name}',
...
def services_view(request):
    ...
    return render(request, 'websites/all_services.html', context)


def ajax_services_list(request):
    """
    Returns is_service=True galleries as JSON, with a cover image resolved
    from photos_gallery_dtls (is_cover_photo -> is_thumbnail_photo -> first).
    """
    galleries = (
        photos_gallery.objects
        .filter(is_service=True)
        .order_by('phgallery_id')
    )

    results = []
    for g in galleries:
        details_qs = g.phgallery2phgdtls.all()

        cover = details_qs.filter(is_cover_photo=True).first()
        if not cover:
            cover = details_qs.filter(is_thumbnail_photo=True).first()
        if not cover:
            cover = details_qs.first()

        image_url = ''
        if cover and cover.photos:
            try:
                image_url = cover.photos.url
            except ValueError:
                image_url = ''
...
def service_details_view(request, phgallery_id):
    
    org_data = org_info.objects.first()
    
    gallery = get_object_or_404(photos_gallery, phgallery_id=phgallery_id, is_service=True)
    photos = gallery.phgallery2phgdtls.all().order_by('-is_cover_photo', 'phgdtls_id')
    ...
    return render(request, 'websites/service_details.html', context)


def all_gallery_photos(request):
    org_data = org_info.objects.first()
    context = {
        'org_data': org_data,
    }
    
    return render(request, 'websites/gallery_details.html', context)

def all_gallery_photos_ajax(request):
    items = photos_gallery_dtls.objects.select_related('phgallery_id').all().order_by('-is_cover_photo', '-ss_created_on')
    ...
    for item in items:
        ...
        data.append({
            'id': item.phgdtls_id,
            'gallery_id': item.phgallery_id_id if item.phgallery_id else None,
            'gallery_title': getattr(item.phgallery_id, 'gallery_title', '') if item.phgallery_id else '',
            'title': item.photos_title or '',
            'description': item.photo_description or '',
            'url': item.photos.url,
            'width': width,
            'height': height,
            'orientation': orientation,
            'is_cover': item.is_cover_photo,
            'is_thumbnail': item.is_thumbnail_photo,
        })
...
def albums_list_page(request):
    org_data = org_info.objects.first()
    albums = photos_gallery.objects.filter(is_album=True).order_by('-ss_created_on')

    album_cards = []
    for album in albums:
        details = album.phgallery2phgdtls.all()
        cover = details.filter(is_cover_photo=True).first() or details.first()

        tags = []
        if album.is_recent_work:
            tags.append('Featured')
        tags.append(album.get_photo_type_display())

        album_cards.append({
            'id': album.phgallery_id,
            'title': album.title or 'Untitled Album',
            'cover_photo': cover.photos.url if cover and cover.photos else None,
            'tags': tags,
        })

    return render(request, 'websites/albums.html', {'albums': album_cards, 'org_data': org_data})


def album_detail_page(request, pk):
    org_data = org_info.objects.first()
    """Renders the static shell only — JS fetches the actual data via AJAX."""
    album = get_object_or_404(photos_gallery, pk=pk, is_album=True)
    return render(request, 'websites/album_detail.html', {'album_id': album.phgallery_id, 'org_data': org_data})


def album_detail_api(request, pk):
    """AJAX endpoint: returns album info + all photos."""
    album = get_object_or_404(photos_gallery, pk=pk, is_album=True)
    photos = album.phgallery2phgdtls.all().order_by('-is_cover_photo', 'phgdtls_id')
    ...
    photos_data = [{
        'id': p.phgdtls_id,
        'title': p.photos_title or '',
        'url': p.photos.url if p.photos else None,
        'is_cover_photo': p.is_cover_photo,
    } for p in photos if p.photos]
