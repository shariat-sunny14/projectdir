...
from user_auth.utils.save_org_list_context import save_org_list_json_for_user
from user_auth.utils.save_branch_list_context import save_branch_list_json_for_user
...
def check_session_status(request):
    ...
    is_expired = expiry_age <= 0 or current_time > expiry_date

    if is_expired:
        try:
            # session থেকে user_id বের করি
            ...
            if user_id:
                ...
                user.is_login_status = False
                user.save(update_fields=['is_login_status'])
        except Exception:
            pass
    ...
    if request.method == 'POST':
        ...
        if user is not None and user.check_password(password):
            ...
            if user.org_id == organization.org_id:
                ...
                save_org_list_json_for_user(user)
                save_branch_list_json_for_user(user, org_id=organization.org_id)
