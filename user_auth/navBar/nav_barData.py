...
def nav_barItemContext(request):
    ...
    if request.user.is_authenticated:
        active_modules = module_list.objects.filter(is_active=True).order_by('module_id')  # Sorted by module_id
        ...
        for access_item in access_list_data:
            ...
            if module_id in active_modules.values_list('module_id', flat=True):
                ...
                # Only add the type if it has at least one active "Form" feature
        ...
        # Remove type_name entries that have no features
        for module_id in list(grouped_data.keys()):
            types_to_remove = [
                type_name for type_name, type_data in grouped_data[module_id]['types'].items()
                if not type_data['features']
            ]
            for type_name in types_to_remove:
                del grouped_data[module_id]['types'][type_name]

    # Sort grouped_data by module_id in ascending order
    grouped_data = dict(sorted(grouped_data.items(), reverse=False))
