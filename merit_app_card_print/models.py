...
class in_merit_position_approval(models.Model):
    ...
    section_id = models.ManyToManyField(in_section, blank=True, related_name='section_id2merit_position')
    ...
    # 
    is_half_roll_sec_change = models.BooleanField(default=False)
    is_half_roll_sec_change_by = models.ForeignKey(User, null=True, blank=True, related_name='change_user_id2merit_position', on_delete=models.DO_NOTHING)
    is_half_roll_sec_change_date = models.CharField(max_length=50, null=True, blank=True)
    # 
...
class in_merit_position_approvaldtls(models.Model):
    ...
    fail_subject_count = models.IntegerField(null=True, blank=True, default=0)
...
class in_merit_position_subjectdtls(models.Model):
    msubdtl_id = models.BigAutoField(primary_key=True, default=1859600000001, editable=False)
    created_date = models.DateField(default=datetime.now, editable=False)
    merit_year = models.IntegerField(default=datetime.now().year, editable=False)
    merit_id = models.ForeignKey(in_merit_position_approval, null=True, blank=True, related_name='merit_id2merit_subjectdtls', on_delete=models.DO_NOTHING)
    reg_id = models.ForeignKey(in_registrations, null=True, blank=True, related_name='reg_id2merit_subjectdtls', on_delete=models.DO_NOTHING)
    subject_id = models.ForeignKey(in_subjects, null=True, blank=True, related_name='subject_id2merit_subjectdtls', on_delete=models.DO_NOTHING)
    subject_name = models.CharField(max_length=200, null=True, blank=True)
    subject_priority = models.IntegerField(null=True, blank=True, default=0)
    subject_marks = models.FloatField(null=True, blank=True, default=0)
    ss_creator = models.ForeignKey(User, null=True, blank=True, related_name='ss_creator2merit_subjectdtls', on_delete=models.DO_NOTHING, editable=False)
    ss_created_on = models.DateTimeField(auto_now_add=True)
    ss_created_session = models.BigIntegerField(null=True, blank=True, default=18596003000001, editable=False)
    ss_modifier = models.ForeignKey(User, null=True, blank=True, related_name='ss_modifier2merit_subjectdtls', on_delete=models.DO_NOTHING)
    ss_modified_on = models.DateTimeField(auto_now=True)
    ss_modified_session = models.BigIntegerField(null=True, blank=True, default=18596004000001, editable=False)

    def save(self, *args, **kwargs):
        trans_data = in_merit_position_subjectdtls.objects.all()

        if trans_data.exists() and self._state.adding:
            last_order = trans_data.latest('msubdtl_id')
            user_session = trans_data.latest('ss_created_session')
            modifier_session = trans_data.latest('ss_modified_session')
            self.msubdtl_id = int(last_order.msubdtl_id) + 1
            self.ss_created_session = int(user_session.ss_created_session) + 1
            self.ss_modified_session = int(modifier_session.ss_modified_session) + 1
        super().save(*args, **kwargs)

    def __str__(self):
        return str(self.msubdtl_id)