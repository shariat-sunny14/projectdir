...
def generate_barcode(request):
    ...
    try:
        ...
        org_name = stock_item.item_id.org_id.org_name
        item_name = stock_item.item_id.item_name
        sales_price = stock_item.item_id.sales_price
        ...
        font = ImageFont.truetype("arial.ttf", 20)  # Adjust font size if necessary
        draw.text((15, barcode_image.height), label_text, fill='black', font=font)
...
def generate_barcode_bulk(request):
    try:
        ...
        for item_id, stock_id in zip(selected_item_ids, selected_stock_ids):
            ...
            org_name = item.item_id.org_id.org_name
            item_id_code = item_instance.item_id
            item_name = item_instance.item_name
            sales_price = item_instance.sales_price
            item_batch = item.item_batch
            ...
            barcode_image = Image.open(io.BytesIO(barcode_img))
            label_text = f'{org_name}\nB: {item_batch}\nN: {item_name}\nP: {sales_price} Tk'
            final_image = Image.new('RGB', (barcode_image.width, barcode_image.height + 100), color='white')
            final_image.paste(barcode_image, (0, 0))
