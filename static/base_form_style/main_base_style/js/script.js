...
//////////////////////////////////////////////////////
const dropdowns = [
    { id: "id_is_class", key: "selected_class_id" },
    { id: "id_is_class_com", key: "selected_class_com" },
    { id: "id_is_section", key: "selected_section_id" },
    { id: "id_is_section_com", key: "selected_section_com" },
    { id: "id_is_shift", key: "selected_shift_id" },
    { id: "id_is_shifts_com", key: "selected_shifts_com" },
    { id: "id_is_groups", key: "selected_groups_id" },
    { id: "id_is_groups_com", key: "selected_groups_com" },
    { id: "id_is_shifts", key: "selected_is_shifts" },
    { id: "id_exam_type_com", key: "selected_exam_type_com" },
    { id: "id_exam_type", key: "selected_id_exam_type" },
];

// Save selected values to localStorage when changed
...
//////////////////////////////////////////////////////
// === Save radio value to localStorage ===
document.querySelectorAll('input[name="is_exams_types"]').forEach(radio => {
    radio.addEventListener("change", function() {
        localStorage.setItem("selected_exam_type", this.value);
    });
});

// === Restore radio value on page load ===
window.addEventListener("DOMContentLoaded", function () {
    const savedValue = localStorage.getItem("selected_exam_type");

    if (savedValue) {
        const radioToCheck = document.querySelector(`input[name="is_exams_types"][value="${savedValue}"]`);
        if (radioToCheck) {
            radioToCheck.checked = true;
        }
    }
});
// Show/hide group dropdown based on class selection
//////////////////////////////////////////////////////

$(document).ready(function () {
    function toggleGroupDropdown() {
        const selectedOption = $('#id_is_class option:selected');
        const allowGroup = selectedOption.attr('data-allow_group'); // get as string

        if (allowGroup === 'True') {
            $('#group_section').show();
        } else {
            $('#group_section').hide();
            $('#id_is_groups').val('').prop('selected', true).trigger('change'); // fully reset
        }
    }
    ...
    // On class dropdown change
    $('#id_is_class').on('change', function () {
        toggleGroupDropdown();
    });
});