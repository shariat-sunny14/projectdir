...
def halfYearlyTabulationReportsAPI(request):
    ...
    is_half_yearly = _bool_or_none(params.get('is_half_yearly'))
    is_yearly = _bool_or_none(params.get('is_yearly'))
    ...
    if is_half_yearly is not None:
        att_qs = att_qs.filter(is_half_yearly=is_half_yearly)
    if is_yearly is not None:
        att_qs = att_qs.filter(is_yearly=is_yearly)
    if is_version == 'english':
    ...
    if is_half_yearly is not None:
        attdtl_qs = attdtl_qs.filter(is_half_yearly=is_half_yearly)
    if is_yearly is not None:
        attdtl_qs = attdtl_qs.filter(is_yearly=is_yearly)
    if is_version == 'english':
