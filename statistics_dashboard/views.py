...
@login_required()
def statisticsTotalSalesReportAPI(request):
    from_date = None
    to_date = None
    ...
    if request.method == "POST":
        from_date = request.POST.get('start_from')
        to_date = request.POST.get('end_from')
...
@login_required()
def statisticsTotalCashCollReportAPI(request):
...
@login_required()
def statisticsTotalDuesReportAPI(request):
    due_from = None
    due_to = None
    ...
    if request.method == "POST":
        due_from = request.POST.get('start_from')
        due_to = request.POST.get('end_from')
        org_id = request.POST.get('org_id')
        branch_id = request.POST.get('branch_id')
        ...
        for invoice in invoices:
            ...
            if total_dues > 0 or total_dues < 0:
                ...
                grand_total_dues = round(grand_total_dues, 2)
                grand_total_cost_amt += total_cost_amt
