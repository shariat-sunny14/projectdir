...
@login_required()
def statisticsAppSummaryManagerAPI(request):
    # SAFE DATE PARSE
    # DATE RANGE PARSE
    ...
    active_sessions = sessions.filter(expire_date__gte=timezone.now()).count()
    expired_sessions = sessions.filter(expire_date__lt=timezone.now()).count()
    ...
    for model in apps.get_models():
        ...
        for field in ["update_date", "create_date", "trans_date"]:
            if hasattr(model, field):
                f = model._meta.get_field(field)

                # Apply only if field is DATE or DATETIME
        ...
        # COUNT COMPUTATION
        # ------------------------------
        submit_count = queryset.count()

        update_count = (
            queryset.filter(update_date__isnull=False).count()
            if hasattr(model, "update_date")
            else 0
        )

        delete_count = (
            queryset.filter(is_deleted=True).count()
