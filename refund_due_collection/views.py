...
def saveDueCollectionAmountAPI(request):
    ...
    due_amt = float(data.get('total_due_amount'))
    payment_amt = float(data.get('total_payment_amt'))
    collection_mode = int(data.get('pay_Collection_mode'))
    pay_mode = int(data.get('pay_mode'))
    # Extracting and parsing invdtl_id and gross_dis arrays
    try:
        invdtl_ids = list(map(int, data.getlist('invdtl_id[]')))
        gross_disc = list(map(float, data.getlist('gross_dis[]')))
    except ValueError as e:
        resp['errmsg'] = f"Invalid data format: {str(e)}"
        return JsonResponse(resp)
    ...
    try:
        ...
        with transaction.atomic():
            ...
            for invdtl_id, gross_dis in zip(invdtl_ids, gross_disc):
                # Update the gross_dis field for the corresponding invdtl_id
                invoicedtl_list.objects.filter(invdtl_id=invdtl_id).update(gross_dis=gross_dis)
...
def saveRefundAmountAPI(request):
    ...
    refund_amt = float(data.get('total_due_amount_ref'))
    payment_amt = float(data.get('total_payment_amt_ref'))
    collection_mode = int(data.get('pay_Collection_mode_ref'))
    pay_mode_ref = int(data.get('pay_mode_ref'))
