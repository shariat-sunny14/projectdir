...
def saveEnrollSetupManagerAPI(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            order_by = request.POST.get('order_by') or 0
            ...
            if enroll_id:
                ...
                enroll.order_by = order_by
                enroll.ss_modifier = request.user
                enroll.save()

                order_list = request.POST.getlist('edit_enroll_order_no_list[]')
                details_list = request.POST.getlist('edit_enroll_dtls_list[]')
                publish_list = request.POST.getlist('edit_is_published_list[]')
                dtls_id_list = request.POST.getlist('edit_enroll_dtls_id_list[]')
                ...
                publish_list = request.POST.getlist('is_published_list[]')
                dtls_id_list = []

                msg = "Enroll created successfully"

            if len(order_list) != len(details_list) or len(order_list) != len(publish_list):
                return JsonResponse({
                    "success": False,
                    "errmsg": "Order, details and publish list length mismatch"
                })
            ...
            for i in range(len(order_list)):
                ...
                is_published = publish_list[i] == '1'

                dtls_id = dtls_id_list[i] if i < len(dtls_id_list) else None

                if dtls_id:
                    EnrollDetails.objects.filter(
                        enrolldtls_id=dtls_id, enroll_id=enroll
                    ).update(
                        order_no=order_no,
                        enroll_dtls=details,
...
def save_enroll_booking(request):
    if request.method == "POST":
        ...
        booking = EnrollBookingDtls.objects.create(
            enroll=enroll,
            full_name=name,
            mobile_no=mobile,
            email=email,
        ...
        # ==========================================
        # ✅ Booking DB তে commit হয়ে গেছে, এখন org_info.email এ
        ...
        try:
            ...
            if org and org.email:
                ...
                subject = f"New Enroll Booking Received - Booking #{booking.booking_id}"
                body = f"""
A new enroll booking has been submitted.

Booking ID: {booking.booking_id}
Enroll: {enroll.enroll_id}
Enroll Title: {enroll.enroll_title}
Full Name: {booking.full_name}
Customer Email: {booking.email}
Mobile No: {booking.mobile_no}
Address: {booking.address}
Course Amount: {booking.course_amt}
