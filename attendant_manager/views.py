...
def studentAttendantListManagerAPI(request):
    ...
    return render(request, 'attendant_manager/attendant_manager_list.html', context)
...
@login_required()
def approvedRollbackAttendantManagerAPI(request):
    attendant_data = {}
    if request.method == 'GET':
        ...
        if attendant_id.isnumeric() and int(attendant_id) > 0:
            attendant_data = in_student_attendant.objects.filter(attendant_id=attendant_id).first()

    context = {
        'attendant_data': attendant_data,
    }
    return render(request, 'attendant_manager/approved_rollback_attendant.html', context)


@login_required()
def approvedRollbackAttendantSubmissionAPI(request):
    ...
    try:
        with transaction.atomic():
            ...
            # ---- Fetch Parent Table ----
            att_data = in_student_attendant.objects.get(attendant_id=attendant_id)

            # ---- Check current year match ----
            current_year = datetime.now().year
            if att_data.attendant_year != current_year:
                resp['errmsg'] = f"Rollback failed! Attendance year ({att_data.attendant_year}) does not match current year ({current_year})."
                return JsonResponse(resp)

            # ---- Update Parent Table ----
            att_data.is_approved = False
            att_data.is_approved_by = None
            att_data.approved_date = None
            att_data.ss_modifier = request.user
            att_data.save()

            # ---- Update Child Table ----
            in_student_attendantdtls.objects.filter(attendant_id=att_data).update(
                is_approved=False,
                is_approved_by=None,
                approved_date=None,
...
def getStudentAttendantListManagerAPI(request):
    ...
    has_access = access_list.objects.filter(
        user_id=user,
        feature_id__feature_page_link='ATTENDANTSAPPROLLBACKACC',
        is_active=True
    ).exists()
