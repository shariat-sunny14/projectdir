...
from . models import invoice_list, invoicedtl_list, item_fav_list, payment_list, rent_others_exps, reward_points, reward_pointsdtls
        ...
        with transaction.atomic():
            ...
            if pay_amt > 0 and collection_mode == 1:
                ...
                if pay_mode in [1, 5, 6, 7, 8]:
                    # because other_exps_amt substract in total collection. other_exps_amt save individually
                    totalPayment = pay_amt - other_exps_amt
                    # Add totalPayment to on_hand_cash
                    cashOnHands.on_hand_cash = F('on_hand_cash') + totalPayment

                ##### pay_mode in [2, 3, 4] is bank deposit amount its not add substract into on_hand_cash.
                ##### bank deposit - other_exps_amt substrct data show into collection report
...
def saveGroceryStoreInvoiceAPI(request):
    ...
    try:
        with transaction.atomic():
            ...
            # Initialize total_balance_amt outside the loop
            total_balance_amt = 0
            ...
            for item_id, store_id, qty, uom_id, price, w_dis, dis, vat_tax, item_name in item_data:
                ...
                # reward point data
                # Calculate total balance for the current item
                item_total_balance = float(qty) * float(price)

                # Accumulate total balance across all items
                total_balance_amt += item_total_balance

                # Reward point logic
                if reg_id and total_balance_amt > 99.99 and int(collection_mode) == 1:
                    # Calculate reward points for the cumulative total balance
                    reward_point_bal = total_balance_amt * 0.02
                    ...
                    rewardPoint.reward_balance = F('reward_balance') + reward_point_bal
                    ...
                    reward_pointsdtls.objects.create(
                        ...
                        branch_id=branch_instance,
                        reg_id=reg_instance,
                        reward_dtls_balance=reward_point_bal,
                        ss_creator=request.user,
                        ss_modifier=request.user,
                    )
            ...
            if float(pay_amt) > 0:
                ...
                if reg_id and int(collection_mode) == 4 :
                    ...
                    adj_rewardPoint.reward_balance = F('reward_balance') - pay_amt
...
def getRewardPointBalanceAPI(request):
    ...
    if request.method == "GET":
        ...
        try:
            org_id = int(request.GET.get('org_id', 0)) if request.GET.get('org_id') else None
        except ValueError:
        ...
        data = [{'reward_balance': point.reward_balance} for point in reward_points_qs]
...
def receipt(request):
    ...
    total_net_collection = (total_collection_amt + adjust_amt_sum) - refund_amt_sum

    # net_due_amt = net_total_amt - pay_amt_sum
    net_due_amt = ((net_total_amt - adjust_amt_sum) - (total_collection_amt - refund_amt_sum))
    # Calculate the net_due_amt and round it to 2 decimal places
    net_due_amt = round(net_due_amt, 0)

    # given amount calculations
    tot_given_amt = PayList.aggregate(total_given=Sum('given_amt'))['total_given'] or 0
