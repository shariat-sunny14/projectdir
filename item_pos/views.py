    ...
    if request.method == 'POST':
        try:
            with transaction.atomic():
                ...
                if not all([org_id, branch_id, reg_id]):
                ...
                # Get or create the reward point record
                reward_point, created = reward_points.objects.get_or_create(
                    reg_id_id=reg_instance.pk,  # Pass the primary key here
                    defaults={
                        'org_id': org_instance,
                        'branch_id': branch_instance,
                ...
                if not created:
                    # Update the reward_balance for an existing record
