...
def item_posAPI(request):
    ...
    stock_recon_access = access_list.objects.filter(
        user_id=user,
        feature_id__feature_page_link='STOCKRECBTNACCESS',
        is_active=True
    ).exists()
    
    store_trans_access = access_list.objects.filter(
        user_id=user,
        feature_id__feature_page_link='STORETRANSBTNACCESS',
        is_active=True
    ).exists()
    ...
    context_pos = {
        'org_list': org_list,
        'sale_data': sale_data,
        'stock_recon_access': stock_recon_access,
        'store_trans_access': store_trans_access,
...
def receipt(request):
    ...
    for field in invoice_list._meta.get_fields():
        ...
        transaction['reg_id'] = sales.reg_id.reg_id if sales.reg_id else None
        transaction['org_id'] = sales.org_id.org_id if sales.org_id else None
    ...
    # inword query
    # Convert total_net_collection to words
    words = ''
    if total_net_collection > 0:
        words = f"{num2words(total_net_collection, lang='en')} Tk. only"
