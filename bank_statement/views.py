...
def bankStatementManagerAPI(request):
    ...
    return render(request, 'bank_statement/bank_statement_list.html', context)
...
def getBankStatementListsAPI(request):
    ...
    deposit_data = daily_bank_statement.objects.filter(filter_kwargs)
...
def addBankStatementModelManageAPI(request):
    ...
    bank_data = bank_lists.objects.filter(is_active=True).all()
    ...
    return render(request, 'bank_statement/add_bank_statement.html', context)
    ...
    try:
        with transaction.atomic():
            ...
            if int(deposits_amt) > 0:
                ...
                cash_on_hands_obj.on_hand_cash -= int(deposits_amt)
