...
def addBankStatementAPI(request):
    ...
    try:
        with transaction.atomic():
            ...
            deposits_amt = float(deposits_amt)
            if deposits_amt > 0:
                ...
                cashOnHands.on_hand_cash = F('on_hand_cash') - deposits_amt
                cashOnHands.save()
...
def addDepositAtMainBranchModelAPI(request):
    ...
    if user.is_superuser:
        ...
        sub_branch_data = branchslist.objects.filter(is_active=True, is_sub_branch=True)
        main_branch_data = branchslist.objects.filter(is_active=True, is_main_branch=True)

    elif user.org_id is not None:
        # If the user is tied to a specific org
        org_list = organizationlst.objects.filter(is_active=True, org_id=user.org_id)
        sub_branch_data = branchslist.objects.filter(is_active=True, org_id=user.org_id, branch_id=user.branch_id, is_sub_branch=True)
        main_branch_data = branchslist.objects.filter(is_active=True, org_id=user.org_id, is_main_branch=True)
    else:
        org_list = []
        sub_branch_data = []
        main_branch_data = []
...
def addDepReceivedAtSubBranchModelAPI(request):
    ...
    if user.is_superuser:
        ...
        org_list = organizationlst.objects.filter(is_active=True, org_id=user.org_id)
        sub_branch_data = branchslist.objects.filter(is_active=True, is_sub_branch=True)
        main_branch_data = branchslist.objects.filter(is_active=True, org_id=user.org_id, branch_id=user.branch_id, is_main_branch=True)
    else:
        # If neither a superuser nor associated with an organization, set organizations to an empty list or handle as needed
        org_list = []
        sub_branch_data = []
