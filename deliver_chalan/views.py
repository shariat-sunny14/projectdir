...
def getDeliveryChalanDataAPI(request):
    ...
    try:
        ...
        chalan_data = delivery_Chalan_list.objects.get(inv_id=inv_id)
        chalan_detail_data = invoicedtl_list.objects.filter(inv_id=inv_id)
        ...
        for detail in chalan_detail_data:
            stores_with_qty = []
...
#         chalan_id = data.get('chalan_id')
#         inv_id = data.get('inv_id')
#         chalan_out_ourceing = data.get('chalan_out_ourceing', False)
#         chalan_direct_or_hold = data.get('chalan_direct_or_hold', False)
#         chalan_hold_approve = data.get('chalan_hold_approve', False)
...
#         out_sourceings = data.getlist('out_sourceing[]')
#         hold_sales = data.getlist('hold_sales[]')
#         hold_approves = data.getlist('hold_approve[]')
...
#                 "out_sourceings": len(out_sourceings),
#                 "hold_sales": len(hold_sales),
#                 "hold_approves": len(hold_approves),
#             }
#             print("List lengths: ", list_lengths)

#             min_length = min(len(chanaldtl_ids), len(invdtl_ids), len(item_ids), 
#                              len(store_names), len(delivery_qtys), 
#                              len(out_sourceings), len(hold_sales), len(hold_approves))

#             with transaction.atomic():
#                 # Update or create the delivery_chalan object
#                 delivery_chalan, created = delivery_Chalan_list.objects.update_or_create(
#                     chalan_id=chalan_id,
#                     defaults={
#                         'inv_id_id': inv_id,
#                         'is_out_ourceing': chalan_out_ourceing,
#                         'is_direct_or_hold': chalan_direct_or_hold,
#                         'is_hold_approve': chalan_hold_approve,
#                         'is_update': True,
#                     }
#                 )
...
#                     item_id = item_ids[i]
#                     store_name = store_names[i]
#                     delivery_qty = float(delivery_qtys[i])

#                     out_sourceing_value = 1 if (i < len(out_sourceings) and out_sourceings[i] == '1') else 0
#                     hold_sales_value = 1 if (i < len(hold_sales) and hold_sales[i] == '1') else 0
#                     hold_approve_value = 1 if (i < len(hold_approves) and hold_approves[i] == '1') else 0

#                     # Check if a record exists with store_name == deliver_store_id
#                     existing_chalandtl = delivery_Chalandtl_list.objects.filter(
#                         chalan_id=delivery_chalan,
#                         invdtl_id_id=invdtl_id,
#                         inv_id_id=inv_id,
#                         item_id_id=item_id,
...
#                     if existing_chalandtl:
#                         # If record exists, sum the deliver_qty and delivery_qty
#                         existing_chalandtl.deliver_qty += delivery_qty
#                         existing_chalandtl.is_out_ourceing = out_sourceing_value
#                         existing_chalandtl.is_direct_or_hold = hold_sales_value
#                         existing_chalandtl.is_hold_approve = hold_approve_value
#                         existing_chalandtl.ss_modifier = request.user
#                         existing_chalandtl.save()
#                     else:
#                         # Create a new record since store_name != deliver_store_id
#                         delivery_chalanDtl = delivery_Chalandtl_list.objects.create(
#                             del_chalan_date=timezone.now(),
#                             chalan_id=delivery_chalan,
#                             invdtl_id_id=invdtl_id,
...
#                             deliver_store_id=store_name,
#                             deliver_qty=delivery_qty,
#                             is_out_ourceing=out_sourceing_value,
#                             is_direct_or_hold=hold_sales_value,
#                             is_hold_approve=hold_approve_value,
#                             is_extra_item=True,  # Set is_extra_item to True for newly created records
#                             ss_creator=request.user,
#                             ss_modifier=request.user,
#                         )
