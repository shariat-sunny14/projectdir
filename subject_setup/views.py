    ...
    try:
        with transaction.atomic():
            ...
            if subjects_id and subjects_id.isnumeric() and int(subjects_id) > 0:
                ...
                checksubjects_no = in_subjects.objects.exclude(subjects_id=subjects_id).filter(Q(subjects_no=data.get('subjects_no')) & Q(org_id=org_instance) & Q(class_id=class_instance) & Q(is_english=True) & Q(is_bangla=True)).exists()
                checksubjects_name = in_subjects.objects.exclude(subjects_id=subjects_id).filter(Q(subjects_name=data.get('subjects_name')) & Q(org_id=org_instance) & Q(class_id=class_instance) & Q(groups_id=groups_instance) & Q(is_english=True) & Q(is_bangla=True)).exists()
                ...
                # Check if the provided subjects_no or subjects_name already exists for other items
                checksubjects_no = in_subjects.objects.filter(Q(subjects_no=data.get('subjects_no')) & Q(org_id=org_instance) & Q(class_id=class_instance) & Q(is_english=True) & Q(is_bangla=True)).exists()
                checksubjects_name = in_subjects.objects.filter(Q(subjects_name=data.get('subjects_name')) & Q(org_id=org_instance) & Q(class_id=class_instance) & Q(groups_id=groups_instance) & Q(is_english=True) & Q(is_bangla=True)).exists()
            ...
            subjects_data.short_name = data.get('short_name')
            ...
            subjects_data.is_pass_marks = data.get('is_pass_marks')
            ...
            subjects_data.is_english = is_english
            subjects_data.is_bangla = is_bangla
            subjects_data.is_applicable_pass_marks = data.get('is_applicable_pass_marks', 0)
            subjects_data.is_not_countable = data.get('is_not_countable', 0)
            subjects_data.is_optional = data.get('is_optional', 0)
...
def getSubjectsDataManagerAPI(request):
    ...
    elif is_version == 'optional':
        filter_kwargs &= Q(is_optional=True)
...
def selectSubjectsDataManagerAPI(request, subjects_id):
    ...
    try:
        ...
        subjectsDtls.append({
            ...
            'is_applicable_pass_marks': subjects_list.is_applicable_pass_marks,
            'is_not_countable': subjects_list.is_not_countable,
            'is_optional': subjects_list.is_optional,
            'is_english': subjects_list.is_english,
            'is_bangla': subjects_list.is_bangla,
        })
