...
def get_totalSum_available_qty(item_id): 
    try:
        # Aggregate stock quantities for the given item_id across all stores
        available_qty = in_stock.objects.filter(item_id=item_id).aggregate(total_qty=Sum('stock_qty'))['total_qty']

        # Return the aggregated quantity, defaulting to 0 if None
